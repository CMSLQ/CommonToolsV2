...
static Double_t A0, sA, B0, sB, MaxSig = 100.;
static Double_t MinLike = 1.e-4, ErrMin = 1.e-4, epsilon = 1.e-4;
static Int_t N;
...
//
Double_t CLA(Double_t ilum, Double_t slum, Double_t eff, Double_t seff, Double_t bck, Double_t sbck, Bool_t gauss = false)
{
...
//
   ...
   Double_t lmax = x[0];
   cout << "Likelihood function is evaluated over [0," << lmax << "]\n";
//
   Double_t CL95A = 0;
...
//
      while (delta > epsilon) 
      {
         ...
         x[0] = lmax;
         delta = (Likelihood(x,p) - MinLike)/MinLike;
         if (delta <= epsilon) cout << "Likelihood function is now being evaluated over [0," << lmax << "]\n";
      }         
//
      TF1 *like = new TF1("Likelihood",Likelihood,0.,lmax,0);
      Double_t Norm = like->Integral(0.,lmax,p,epsilon);
//
      x1 = 0;
      x2 = lmax;
      xmax = (x1 + x2)/2.;
...
//
      while (fabs(delta) > epsilon && (x2 - x1)/(x2 + x1) > epsilon) 
      {
//         cout << "x1,x2,delta = " << x1 << ", " << x2 << ", " << delta << endl;
         if (delta < 0) x1 = xmax;
         else x2 = xmax;
         xmax = (x1 + x2)/2.;
         delta = like->Integral(0,xmax,p,epsilon)/Norm - 0.95;
      ...
      delete like;
      Double_t s95 = xmax*Poisson(B0,N);
      CL95A += s95;
      cout << "n = " << N << "; 95% C.L. = " << xmax << " pb; weighted 95% C.L. = " << s95 << " pb; running <s95> = " << CL95A << " pb" << endl;
//
      if (s95 < CL95A*ErrMin) break;
   }
   cout << "Lower bound on n has been found at " << N << endl;
//
   lGauss = gauss;
   for (N = B0+1; ; N++)
   {
...
//
      ...
      Double_t s95 = xmax*Poisson(B0,N);
      CL95A += s95;
      cout << "n = " << N << "; 95% C.L. = " << xmax << " pb; weighted 95% C.L. = " << s95 << " pb; <s95> = " << CL95A << " pb" << endl;
//
      if (s95 < CL95A*ErrMin) break;
   }
   cout << "Upper bound on n has been found at " << N << endl;
//
...
//
    ...
    B = par[0];
    sigma = par[1];
    if (lGauss) return 1./(2.*TMath::Pi()*sqrt(n))/sA*exp(-(x[0]-A0)*(x[0]-A0)/2./sA/sA)*exp(-(n-B-sigma*x[0])*(n-B-sigma*x[0])/2./n);
