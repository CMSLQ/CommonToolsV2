/* --------------------------------------------------------------------------------
 ...
 This version is designed to combine the LQ channels with different beta sensitivity
...
.L LQcomb.C++
 ...
 Modified by Greg Landsberg; v2.0 November 17, 2010 - LQ combination
-------------------------------------------------------------------------------- */
#include <iostream>
#include <stdlib.h>
#include "TMath.h"
...
void LQ2comb()
{
 ...
 // Mass points
 Double_t m[9] = {200, 250, 280, 300, 320, 340, 400, 450, 500};
 ...
 Double_t db2[9] = {0.22, 0.22, 0.22, 0.22, 0.22, 0.22, 0.22, 0.22, 0.22};
...
//    cout << "Expected 95% C.L. lower limit on LQ mass = " << m_expected[j] << " GeV"<< endl;
 ...
 Double_t beta_range_lowest[2] = {0.05, 0.12};
 Int_t nPts_lowest = 6;
...
//
        ...
        {
                if (B0 > 0) sigma_b = TMath::Log(1. + epsbck);
                sigma_a = TMath::Log(1. + epslum);
        ...
        else if (nuisanceModel == 2) // Gamma distribution is used
        {
                ...
                if (sA > 0)
                {
                        sigma_a = 1. + 1./epslum/epslum;
                        tau_a = A0/sA/sA;
                }
                if (sB > 0)
                {
                        sigma_b = 1. + 1./epsbck/epsbck;
                        tau_b = B0/sB/sB;
                }
        }
        ...
        // Do some iterations to determine in which cross-section range 0 to xmax the likelihood function is sufficiently
...
// (N is a global parameter. The parameter p is dummy).
{
        ...
        if (I == 0)
        {
                ...
                retval = out->Integral(low,B0+5.*sB,par,epsilon);
        } else {
                Double_t low = 0., high = B0 + 5.*sB, tmp = 1.;
                while (tmp > epsilon)
                {
                        tmp = out->Integral(low,high,par,epsilon);
                        retval += tmp;
                        low = high;
                        high *= 2;
                }
...
// values of this parameter, weighted by the a priori probability that it is correct.
// This function makes use of function Inner, which also takes into account uncertainty in lumi*effi.
{
        Double_t retval;
        Double_t par[2];
...
Double_t CLA(Double_t ilum1, Double_t ilum2, Double_t epslum, Double_t eps1, Double_t eps2, Double_t bck1, Double_t bck2, Double_t epsbck, Double_t bcharged, Int_t bckint)
{
        ...
        for (i = bck1; i >= 0; i--)
        {
                Double_t factor = Poisson(bck1,i);
                Int_t contrib = 0;
                for (j = bck2; j >= 0; j--)
                {
