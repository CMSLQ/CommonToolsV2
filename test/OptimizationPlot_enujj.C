...
// root -l -q OptimizationPlot_enujj.C+\(100\)
...
void OptimizationPlot_enujj(int nbins = 100)
{
  ...
  TH1D Signal;
  ((TH1D*)file1.Get("histo1D__LQenujj_M200__Optimizer"))->Copy(Signal); Signal.Sumw2();
  TH1D Backgnd;
  ...
  TH1D Backgnd_QCD;
  ((TH1D*)file2.Get("histo1D__DATA__Optimizer"))->Copy(Backgnd_QCD); Backgnd_QCD.Sumw2();

  ////////////////////////////////////////////////
  // Parameters needed when using the CLA.C method
  ////////////////////////////////////////////////
  double eff; // signal efficiency
  double eff_err_rel = 0.10; // Relative error on signal efficiency
  double Nb_err_rel = 0.40; // Relative error on background, dominated by JES
  double ILum = 30; // pb-1
